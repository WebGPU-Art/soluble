...
fn fragment_main(vx_out: VertexOut) -> @location(0) vec4<f32> {
  ...
  // Warm gold background — contrasts nicely with icosahedral 5-fold patterns
  var total_color = vec4<f32>(0.18, 0.08, 0.02, 1.0);
  ...
  for (var times = 0u; times < max_reflect_times + 1u; times++) {
    ...
    for (var i = 0u; i < segments_size; i = i + 1u) {
      ...
      let color_scale = 1.5 / pow(distance * 0.07 + 0.01, 1.8) / f;

      // Warm amber tint for the light segments
      total_color += vec4<f32>(0.022, 0.012, 0.004, 0.0) * color_scale;
      total_color  = min(total_color, vec4<f32>(0.9, 0.7, 0.3, 1.0));
    }

    if hit_mirror {
      if rand11(dot(vx_out.uv, vec2f(127.1, 311.7)) + f32(times) * 43.7) < 0.15 { break; }
      total_color     += vec4<f32>(0.012, 0.008, 0.022, 0.0);
