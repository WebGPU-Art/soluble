...
fn mobius_slice(z: vec2<f32>, time: f32, phase: f32) -> vec2<f32> {
  let tm = 0.18 * time;
  let a = vec2<f32>(1.0, 0.16 * sin(tm + 0.7 * phase));
  let b = vec2<f32>(
    0.22 * cos(tm * 1.21 - 0.45 * phase),
    0.18 * sin(tm * 0.93 + 0.65 * phase)
  );
  let c = vec2<f32>(
    0.16 * cos(tm * 0.57 + 0.8 * phase),
    -0.10 + 0.07 * sin(tm * 0.71 - 0.55 * phase)
  );
  let d = vec2<f32>(1.0, 0.0);
  return complex_div(complex_mul(a, z) + b, complex_mul(c, z) + d);
}

fn helix_warp3(p: vec3<f32>, time: f32, phase: f32) -> vec3<f32> {
  var q = p;
  let axial = 0.62 * tanh_approx(0.95 * q.y) + 0.28 * sin(0.47 * time + 1.4 * phase + 2.2 * length(q.xz));
  let radial = 0.18 * sin(0.63 * time - 1.2 * phase + 2.6 * q.y);
  q.xz = rotate2(q.xz, axial);
  q.xy = rotate2(q.xy, radial + 0.22 * axial);
  return q;
}

fn helix_warp4(p: vec4<f32>, time: f32, phase: f32) -> vec4<f32> {
  var q = p;
  let axial = 0.52 * tanh_approx(1.05 * q.y) + 0.24 * sin(0.58 * time - 0.7 * phase + 2.0 * q.z);
  let ribbon = 0.30 * sin(0.44 * time + 0.9 * phase + 2.7 * q.x);
  q.xz = rotate2(q.xz, axial);
  q.zw = rotate2(q.zw, -0.78 * axial + ribbon);
  q.xw = rotate2(q.xw, 0.34 * axial - 0.18 * ribbon);
  q.yw = rotate2(q.yw, 0.22 * ribbon + 0.16 * length(q.xz));
  return q;
}
...
fn cluster(p: vec3<f32>, time: f32, phase: f32, thickness: f32, radius: f32) -> MapData {
  ...
  q = helix_warp3(q, time, phase);
  q.xy = mobius_slice(q.xy * 0.80, time, phase + 0.28 * q.y) / 0.80;
  ...
  pp = helix_warp4(pp, time + 1.6, phase);
  pp.xw = mobius_slice(pp.xw * 0.74, time + 1.3, -phase - 0.45 * q.z) / 0.74;
  pp.zw = mobius_slice(pp.zw * 0.68, time + 2.0, phase + 0.35 * q.x) / 0.68;
